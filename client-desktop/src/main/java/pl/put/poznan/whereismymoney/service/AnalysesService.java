...
public class AnalysesService {
    ...
    @Inject
    public AnalysesService(BudgetDao budgetDao, SessionManager sessionManager) {
    ...
    public BigDecimal getIncome(Category category, LocalDate from, LocalDate to) {
        return getTransactionAmount(category, from, to, TransactionType.INCOME);
    }
    ...
    private BigDecimal getTransactionAmount(Category category, LocalDate from, LocalDate to, TransactionType transactionType) {
        ...
        Iterator<Transaction> iterator = category.getRelatedTransactions().stream()
